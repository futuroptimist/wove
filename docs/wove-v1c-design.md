...
- **Workpiece Support Bed** – Removable plate with magnetic anchors to hold fabric while enabling
  ...
  illustrate the recommended swap order. The sweep runs clockwise so operators mirror the same
  plate-change choreography used on the production floor.
...
- **Hook Carriage**: Modular interface accepting either a rotary hook drive (geared stepper) or a
  ...
  spool choreography showcased in the hologram. The electronics bay exhaust now exposes its spinning
  blades in the hologram so contributors can confirm airflow hardware before printing the panel.
...
extrusion cues. When the preview is paused, the countdown ribbon stays pinned with a reminder so
operators know timing is frozen until the loop resumes.
...
operators a visible heads-up in the hologram before the spool spins. Countdown cues now read yarn
feed pulses from extrusion deltas so the ribbon, overlay, and hologram stay aligned even when feed
comments change between planner exports.
