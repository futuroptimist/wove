...
const footstepControllers = [];
const filamentSensorControllers = [];
const thermistorConduitControllers = [];
let thermistorConduitTone = 'info';
const insertGridControllers = [];
...
const automationTracerPoint = new THREE.Vector3();
const automationLightOffset = new THREE.Vector3(0, 0.18, 0);
const stepBadgeControllers = [];
const anchorPulseControllers = [];
let travelEnvelopeController = null;
...
const defaultPatternPreviewDurationSeconds = 14;
...
const spoolCycleSegments = [];
const spoolPrePulseSettings = {
  speed: 2.8,
  phaseOffset: Math.PI / 10,
};
let lastSpoolCountdownSummary = spoolProgressCountdownFallbackMessage;
...
let spoolCountdownLabelController = null;
const fanControllers = [];
const statusIndicatorControllers = [];
const servoActuatorControllers = [];
const servoSignalControllers = [];
const controllerStackGlowControllers = [];
let plannerUploadResetTimer = null;
const plannerUploadLimitBytes = 10 * 1024 * 1024;
let yarnExtrusionActive = false;
let cableChainNextFeedSeconds = null;
let cableChainFollowingFeedSeconds = null;
let cableChainRemainingFeeds = null;
let cableChainCycleTimingDetail = null;
let patternPlannerEvents = [];
let patternPreviewGroup = null;
let patternPreviewContainer = null;
let plannerBounds = null;
let machineProfileBounds = null;
let machineProfileMissingAxes = [];
let currentMachineProfile = null;
let tensionCalibration = null;
let patternPreviewDurationSeconds = defaultPatternPreviewDurationSeconds;
let patternPreviewPaused = false;
let patternPreviewElapsed = 0;
let lastPatternStepIndex = -1;
let lastPatternProgress = -1;
let spoolProgressRatio = 0;
let spoolCycleProgressRatio = 0;
let spoolProgressTone = 'neutral';
let boundsComparison = null;
...
const machineProfileEnvelopeFallbackMessage =
  'Machine profile envelope unavailable. Include travel_min_mm and '
  + 'travel_max_mm for each axis.';
const machineProfileEnvelopeMissingAxesPrefix =
  'Machine profile envelope unavailable — add travel_min_mm and travel_max_mm for: ';
const boundsComparisonFallbackMessage =
  'Bounds comparison unavailable. Export planner bounds and machine profile limits to compare.';
...
const heatedBedStatusFallbackMessage =
  'Thermistor conduit reserved for the heater upgrade — planner status drives the '
  + 'glow along the wiring path.';
const heatedBedMetadataMissingMessage =
  'Heated bed conduit metadata missing — include heated_bed_conduit defaults to drive the glow.';
const heatedBedRouteFallbackMessage =
...
const yarnFlowRateFallbackMessage =
...
const yarnFlowCalibrationFallbackMessage =
  'Calibration: Awaiting tension sensor calibration metadata.';
const yarnFlowTensionFallbackMessage =
...
function deriveFileNameFromSource(value) {
  if (typeof value !== 'string') {
    return null;
  }
  const trimmed = value.trim();
...
function createCableChainBillboard() {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 256;
  const context = canvas.getContext('2d');
  ...
  const toneStyles = {
    ready: { accent: '#79f2d0', text: '#e6fff7' },
    info: { accent: '#9bb7ff', text: '#e5ebff' },
    warning: { accent: '#ffcf7d', text: '#fff2dc' },
    neutral: { accent: '#d3ddff', text: '#f5f7ff' },
  };

  let lastKey = '';

  function drawPanel({ header, lines, tone }) {
    ...
    context.fillStyle = 'rgba(12, 16, 34, 0.84)';
    context.fill();
    context.lineWidth = 4;
    ...
    context.fillText(header, canvas.width / 2, paddingY + 56);

    context.fillStyle = palette.text;
    context.font = '44px "Segoe UI", sans-serif';
    const startY = paddingY + 112;
    const spacing = 46;
  ...
  function updatePanel({ header, lines, tone }) {
    const normalizedLines = Array.isArray(lines) ? lines.map((line) => String(line)) : [''];
    const key = `${header}__${normalizedLines.join('\n')}__${tone}`;
    if (key === lastKey) {
      return;
    }
    drawPanel({ header, lines: normalizedLines, tone });
    lastKey = key;
  }

  updatePanel({
    header: 'Cable Chain',
    lines: ['Countdown: Awaiting planner preview…', 'Remaining feeds: —'],
    tone: 'neutral',
  });
...
function createSpoolCountdownLabel() {
  const canvas = document.createElement('canvas');
  canvas.width = 512;
  canvas.height = 200;
  const context = canvas.getContext('2d');
  ...
  function drawPanel(lines, tone) {
    ...
    context.fillStyle = 'rgba(12, 16, 34, 0.88)';
    context.fill();
    context.lineWidth = 4;
    context.strokeStyle = palette.accent;
    context.stroke();
    ...
    const totalHeight = lineCount * baseFontSize + (lineCount - 1) * 10;
    const startY = paddingY + height / 2 - totalHeight / 2 + baseFontSize / 2;
    lines.forEach((line, index) => {
      context.font = `${baseFontSize}px "Segoe UI", sans-serif`;
      context.fillText(line, canvas.width / 2, startY + index * (baseFontSize + 10));
    });
  ...
  function updatePanel({ line, lines, tone }) {
    const normalizedTone = tone ?? 'neutral';
    const detailLines = Array.isArray(lines)
      ? lines.map((entry) => String(entry))
      : [String(line ?? spoolProgressCountdownFallbackMessage)];

    const key = `${detailLines.join('\n')}__${normalizedTone}`;
    if (key === lastKey) {
      return;
    }

    drawPanel(detailLines, normalizedTone);
    lastKey = key;
  }

  updatePanel({ line: spoolProgressCountdownFallbackMessage, tone: 'neutral' });
...
function normalizeTensionCalibration(defaults) {
  if (!defaults || typeof defaults !== 'object') {
    return null;
  }

  const calibration =
    defaults.tension_sensor_calibration ??
    defaults.tension_calibration ??
    null;

  const candidatePoints = Array.isArray(calibration?.pairs)
    ? calibration.pairs
  ...
  candidatePoints.forEach((entry) => {
    if (Array.isArray(entry) && entry.length >= 2) {
      ...
      if (normalizedReading !== null && normalizedGrams !== null && normalizedGrams >= 0) {
        points.push({ reading: normalizedReading, grams: normalizedGrams });
      }
      return;
    }

    if (entry && typeof entry === 'object') {
      const normalizedReading = coerceFiniteNumber(entry.reading ?? entry.sensor);
      const normalizedGrams = coerceFiniteNumber(entry.grams ?? entry.force_grams);
      if (normalizedReading !== null && normalizedGrams !== null && normalizedGrams >= 0) {
        points.push({ reading: normalizedReading, grams: normalizedGrams });
      }
    }
  ...
  const ordered = points
    ...
    .filter((point, index, array) => {
      ...
      return point.reading > previous.reading && point.grams > previous.grams;
    });

  return ordered.length >= 2 ? ordered : null;
}

function estimateTensionFromSensor(reading, calibration) {
  if (reading === null || !Array.isArray(calibration) || calibration.length < 2) {
    return null;
  }
  ...
  let clamped = false;
  let safeReading = reading;

  if (reading <= lower.reading) {
    safeReading = lower.reading;
    clamped = reading < lower.reading;
  } else if (reading >= upper.reading) {
    safeReading = upper.reading;
    clamped = reading > upper.reading;
  }

  for (let index = 0; index < sorted.length - 1; index += 1) {
    const start = sorted[index];
    const end = sorted[index + 1];
    if (safeReading >= start.reading && safeReading <= end.reading) {
      const span = end.reading - start.reading;
      const ratio = span > 0 ? (safeReading - start.reading) / span : 0;
      const grams = THREE.MathUtils.lerp(start.grams, end.grams, ratio);
      return { grams, clamped, min: lower.reading, max: upper.reading };
    }
  }
...
function normalizeMachineProfileBounds(machineProfile) {
  if (!machineProfile || typeof machineProfile !== 'object') {
    return null;
  }

  const axes = machineProfile.axes && typeof machineProfile.axes === 'object'
    ? machineProfile.axes
    : null;
  if (!axes) {
    return null;
  ...
  Object.entries(axes).forEach(([name, axis]) => {
    ...
    const min = coerceFiniteNumber(axis.travel_min_mm ?? axis.min ?? axis.min_mm);
    const max = coerceFiniteNumber(axis.travel_max_mm ?? axis.max ?? axis.max_mm);
    if (min === null || max === null || max < min) {
      return;
    }
    if (['x', 'y', 'z', 'e', 'extrusion'].includes(key)) {
      normalized[key === 'extrusion' ? 'e' : key] = { min, max };
    }
  });
...
function computeMissingMachineProfileAxes(machineProfile) {
  if (!machineProfile || typeof machineProfile !== 'object') {
    return ['X', 'Y', 'Z', 'E'];
  }
  ...
  const expectedAxes = ['x', 'y', 'z', 'e'];
  const missing = [];

  expectedAxes.forEach((axisKey) => {
    const axis = axes[axisKey] ?? axes[axisKey.toUpperCase()] ?? null;
    const min = coerceFiniteNumber(axis?.travel_min_mm ?? axis?.min ?? axis?.min_mm);
    const max = coerceFiniteNumber(axis?.travel_max_mm ?? axis?.max ?? axis?.max_mm);
    if (!axis || min === null || max === null || max < min) {
      missing.push(axisKey.toUpperCase());
    }
  });
...
function buildBoundsHoverText(bounds) {
  ...
  if (bounds.e || bounds.extrusion) {
    const extrusionBounds = bounds.e ?? bounds.extrusion;
    segments.push(
      `E ${extrusionBounds.min.toFixed(1)}–${extrusionBounds.max.toFixed(1)} mm`,
    );
  }
...
function buildMachineProfileEnvelopeText(bounds, missingAxes = []) {
  const missingAxisList = Array.isArray(missingAxes)
    ? missingAxes.filter((axis) => typeof axis === 'string' && axis.trim().length > 0)
    : [];

  if (missingAxisList.length > 0) {
    return `${machineProfileEnvelopeMissingAxesPrefix}${missingAxisList.join(', ')}.`;
  }

  if (!bounds) {
    return machineProfileEnvelopeFallbackMessage;
  }
  ...
  if (bounds.e) {
    segments.push(`E ${bounds.e.min.toFixed(1)}–${bounds.e.max.toFixed(1)} mm`);
  }
  ...
  return `Machine profile envelope: ${segments.join(' · ')}`;
}
...
function updateBoundsComparisonPanel(plannerBounds, machineBounds, comparison = null) {
  ...
  if (details.exceedingAxes?.length) {
    const segments = details.exceedingAxes.map((axis) => {
      const axisDetail = details.details?.[axis];
      if (!axisDetail) {
        return axis.toUpperCase();
      }

      const { planner, machine, overrunLow, overrunHigh } = axisDetail;
      const issues = [];
      if (overrunLow) {
        issues.push(
          `min ${planner.min.toFixed(1)} mm < machine ${machine.min.toFixed(1)} mm`,
        );
      }
      if (overrunHigh) {
        issues.push(
          `max ${planner.max.toFixed(1)} mm > machine ${machine.max.toFixed(1)} mm`,
        );
      }
...
function updateTravelEnvelope(planner, machine, comparisonDetails = null) {
  if (!travelEnvelopeController) {
    return;
  }
  ...
  const {
    ...
    plannerZHalos,
    group,
  } = travelEnvelopeController;

  const machineDimensions = machine
    ? buildTravelEnvelopeDimensions(machine, fallbackSpan)
    : null;
  const plannerDimensions = planner
    ? buildTravelEnvelopeDimensions(planner, fallbackSpan)
    : null;

  const machineVisible = Boolean(machineDimensions);
  const plannerVisible = Boolean(plannerDimensions);

  machineOutline.visible = machineVisible;
  plannerFill.visible = plannerVisible;
  machineFill.visible = machineVisible;
  machineLabel.visible = machineVisible;
  plannerOutline.visible = plannerVisible;
  plannerLabel.visible = plannerVisible;
  if (plannerZHalos?.group) {
    plannerZHalos.group.visible = plannerVisible;
  }
  group.visible = machineVisible || plannerVisible;

  if (!machineVisible && !plannerVisible) {
    return;
  }

  const missingMachineEnvelope = Boolean(comparisonDetails?.missingMachine);
  const plannerExceedsMachine = Boolean(comparisonDetails?.exceedingAxes?.length);
  const warningTone = missingMachineEnvelope || plannerExceedsMachine;

  const machineColor = warningTone ? 0xffc27d : 0x5fffd2;
  const machineFillColor = warningTone ? 0xffb173 : 0x2cf9c5;
  const machineBaseOpacity = warningTone ? 0.18 : 0.12;
  machineOutline.material.color.setHex(machineColor);
  machineFill.material.color.setHex(machineFillColor);
  machineFill.material.opacity = machineBaseOpacity;

  const plannerColor = plannerExceedsMachine ? 0xff9b6b : 0x9ee5ff;
  const plannerFillColor = plannerExceedsMachine ? 0xffc58f : 0x4fb2ff;
  const plannerBaseOpacity = plannerExceedsMachine ? 0.14 : 0.08;
  plannerOutline.material.color.setHex(plannerColor);
  plannerFill.material.color.setHex(plannerFillColor);
  plannerFill.material.opacity = plannerBaseOpacity;

  if (Array.isArray(boundsCageControllers)) {
    for (let i = 0; i < boundsCageControllers.length; i++) {
      const controller = boundsCageControllers[i];
      if (!controller || !controller.material) {
        continue;
      }
      if (controller.material === machineFill.material) {
        controller.baseOpacity = machineBaseOpacity;
      } else if (controller.material === plannerFill.material) {
        controller.baseOpacity = plannerBaseOpacity;
      }
    }
  }

  if (machineDimensions) {
    const width = Math.max(machineDimensions.width * mmToScene, 0.02);
    const depth = Math.max(machineDimensions.depth * mmToScene, 0.02);
    const height = Math.max(machineDimensions.height * mmToScene, 0.08);
    machineOutline.scale.set(width, height, depth);
    machineFill.scale.set(width, height, depth);
    const machineY = height / 2;
    machineOutline.position.set(0, machineY, 0);
    machineFill.position.set(0, machineY, 0);
    machineLabel.position.set(0, height + 0.16, 0);
  }

  if (plannerDimensions) {
    const width = Math.max(plannerDimensions.width * mmToScene, 0.02);
    const depth = Math.max(plannerDimensions.depth * mmToScene, 0.02);
    const height = Math.max(plannerDimensions.height * mmToScene, 0.08);
    plannerOutline.scale.set(width * 0.98, height * 0.98, depth * 0.98);
    plannerOutline.position.set(0, height / 2 + 0.04, 0);
    plannerFill.scale.set(width * 0.98, height * 0.98, depth * 0.98);
    plannerFill.position.set(0, height / 2 + 0.04, 0);
    plannerLabel.position.set(0, height + 0.2, 0);

    if (plannerZHalos?.group && plannerZHalos.min && plannerZHalos.max) {
      const haloScaleX = Math.max(width * 0.52, 0.08);
      const haloScaleZ = Math.max(depth * 0.52, 0.08);
      plannerZHalos.min.scale.set(haloScaleX, 1, haloScaleZ);
      plannerZHalos.max.scale.set(haloScaleX, 1, haloScaleZ);
      plannerZHalos.min.userData.baseScale = { x: haloScaleX, y: 1, z: haloScaleZ };
      plannerZHalos.max.userData.baseScale = { x: haloScaleX, y: 1, z: haloScaleZ };
      plannerZHalos.min.position.set(0, 0.05, 0);
      plannerZHalos.max.position.set(0, Math.max(height - 0.05, 0.05), 0);
    }
  }
}
...
function updatePlannerDefaultsPanel(defaults) {
  ...
  const rowSpacing = Number(data?.row_spacing_mm);
  if (Number.isFinite(rowSpacing)) {
    entries.push({ label: 'Row spacing', value: `${rowSpacing.toFixed(1)} mm` });
  }
  ...
  const heatedConduit = data && typeof data === 'object' ? data.heated_bed_conduit : null;
  const heatedStatus =
    heatedConduit && typeof heatedConduit.status === 'string'
      ? heatedConduit.status.trim()
      : null;
  const heatedRoute =
    heatedConduit && typeof heatedConduit.route === 'string'
      ? heatedConduit.route.trim()
      : null;
  if (heatedStatus) {
    entries.push({ label: 'Heated bed conduit', value: heatedStatus });
  }

  updateHeatedBedPanel({
    statusLine: heatedStatus,
    routeLine: heatedRoute,
    metadataAvailable: Boolean(heatedConduit),
  });
...
function buildPlannerMetadata(metadata, events) {
  ...
  if (source) {
    ...
    const fileNameCandidate =
      source.fileName ?? source.file_name ?? source.filename ?? null;
    const normalizedFileName =
      normalizeFileName(fileNameCandidate) || deriveFileNameFromSource(source.source);
    if (normalizedFileName) {
      normalized.fileName = normalizedFileName;
    }

    const durationCandidate =
      source.duration_seconds ?? source.durationSeconds ?? source.loop_seconds ?? null;
    const parsedDuration = Number(durationCandidate);
    if (Number.isFinite(parsedDuration) && parsedDuration > 0) {
      normalized.durationSeconds = parsedDuration;
    }

    const sizeCandidate = source.fileSizeBytes ?? source.file_size_bytes ?? null;
    const parsedSize = Number(sizeCandidate);
    if (Number.isFinite(parsedSize) && parsedSize > 0) {
      normalized.fileSizeBytes = parsedSize;
    }
  }
...
function updatePlannerMetadataPanel(metadata) {
  ...
  if (Number.isFinite(data?.durationSeconds)) {
    const duration = Math.max(0, data.durationSeconds);
    entries.push({ label: 'Preview duration', value: `${duration.toFixed(1)} s` });
  }
...
function setPatternPreviewPaused(paused) {
...
function applyHeatedBedConduitTone(tone) {
  thermistorConduitTone = tone || 'info';
  ...
  const settings = toneSettings[thermistorConduitTone] || toneSettings.info;

  thermistorConduitControllers.forEach((controller) => {
    const defaults = controller.defaults ?? {};
    const baseline =
      controller.baseline ??
      (controller.baseline = {
        baseIntensity: controller.baseIntensity ?? 0,
        activeIntensity: controller.activeIntensity ?? 0,
        baseOpacity: controller.baseOpacity ?? 0,
        activeOpacity: controller.activeOpacity ?? 0,
        baseLight: controller.baseLight ?? 0,
        activeLight: controller.activeLight ?? 0,
        shimmerSpeed: controller.shimmerSpeed ?? 1,
        countdownBias: controller.countdownBias,
      });
    const color = settings.color ?? defaults.color;
    const emissive = settings.emissive ?? defaults.emissive;

    if (controller.material) {
      if (color !== undefined && color !== null && controller.material.color) {
        controller.material.color.setHex(color);
      } else if (defaults.color !== undefined && controller.material.color) {
        controller.material.color.setHex(defaults.color);
      }

      if (emissive !== undefined && emissive !== null && controller.material.emissive) {
        controller.material.emissive.setHex(emissive);
      } else if (defaults.emissive !== undefined && controller.material.emissive) {
        controller.material.emissive.setHex(defaults.emissive);
      }
    }

    const intensityScale = settings.intensityScale ?? 1.0;
    const lightScale = settings.lightScale ?? 1.0;
    const shimmerScale = settings.shimmerScale ?? 1.0;

    controller.baseIntensity = baseline.baseIntensity * intensityScale;
    controller.activeIntensity = baseline.activeIntensity * intensityScale;
    controller.baseOpacity = baseline.baseOpacity;
    controller.activeOpacity = baseline.activeOpacity;
    controller.baseLight = baseline.baseLight * lightScale;
    controller.activeLight = baseline.activeLight * lightScale;
    controller.shimmerSpeed = baseline.shimmerSpeed * shimmerScale;
    controller.countdownBias =
      settings.countdownBias ?? baseline.countdownBias ?? controller.countdownBias;
  });
}
...
function applySpoolProgressTone(tone) {
  ...
  if (tone === spoolProgressTone) {
    return;
  }

  spoolProgressTone = tone;
  const palette = spoolProgressTonePalette[tone];
  spoolProgressSegments.forEach(({ material }) => {
    if (!material) {
      return;
    }
...
function updateCableChainBillboard({
  nextSeconds,
  followingSeconds,
  remainingFeeds,
  cycleTimingDetail,
  ...
  if (!cableChainBillboardController) {
    return;
  }
  ...
  if (typeof nextSeconds === 'number') {
    const countdownText = `Next chase pulse in ${nextSeconds.toFixed(1)} s.`;
    lines.push(countdownText);
    tone = extrusionActive ? 'ready' : tone;
  } else {
    lines.push('Countdown: Awaiting planner preview…');
  }

  if (typeof followingSeconds === 'number') {
    lines.push(`Following pulse in ${followingSeconds.toFixed(1)} s.`);
  }

  if (typeof remainingFeeds === 'number') {
    const feedLabel =
      remainingFeeds > 0
        ? `Remaining feeds: ${remainingFeeds}`
        : 'Remaining feeds: Queue clear.';
    lines.push(feedLabel);
    tone = remainingFeeds > 0 ? 'info' : 'ready';
  } else {
    lines.push('Remaining feeds: Awaiting planner preview…');
  }

  if (cycleTimingDetail) {
    lines.push(cycleTimingDetail);
  }

  cableChainBillboardController.update({
    header: 'Cable Chain',
    lines,
    tone,
  });
...
function computeFeedCountdowns(preview, feedStepIndices, progressRatio, durationSeconds) {
  const cumulativeDistances =
    preview && Array.isArray(preview.cumulativeDistances)
      ? preview.cumulativeDistances
      : null;
  const totalDistance =
    preview && Number.isFinite(preview.totalDistance) ? preview.totalDistance : null;
  const totalDuration = Number.isFinite(durationSeconds) ? durationSeconds : null;

  if (
    !cumulativeDistances ||
    !Number.isFinite(totalDistance) ||
    totalDistance <= 0 ||
    !Array.isArray(feedStepIndices) ||
    feedStepIndices.length === 0 ||
  ...
  feedStepIndices.forEach((feedStep) => {
  ...
  return {
    countdowns,
    next: countdowns[0] ?? null,
    following: countdowns[1] ?? null,
  };
}

function formatBillboardFeedCountdown(next, following, remainingFeedCount) {
  if (typeof remainingFeedCount === 'number' && remainingFeedCount <= 0) {
    return 'Next feeds: Queue clear.';
  }

  if (!next) {
    return spoolProgressCountdownFallbackMessage;
  }

  const segments = [
    `Next feed in ${next.delta.toFixed(1)} s (#${next.step + 1})`,
  ];

  if (following && (!remainingFeedCount || remainingFeedCount > 1)) {
    segments.push(
      `Following in ${following.delta.toFixed(1)} s (#${following.step + 1})`,
    );
  }

  return `Next feeds: ${segments.join(' · ')}`;
}
...
function resetYarnFlowPanel() {
  ...
  if (spoolCountdownLabelController) {
    ...
    lastSpoolCountdownToneSource = 'neutral';
    lastSpoolCountdownExtrusionActive = false;
    refreshSpoolCountdownLabel();
  }

  applySpoolProgressTone('neutral');
  updateCableChainBillboard({
    nextSeconds: null,
    followingSeconds: null,
    remainingFeeds: null,
...
function refreshSpoolCountdownLabel() {
  ...
  const countdownTone = patternPreviewPaused
    ? 'warning'
...
function updateYarnFlowPanel(
  ...
  const tensionGrams = coerceFiniteNumber(event.tensionGrams);
  const tensionReading = coerceFiniteNumber(event.tensionSensorReading);
  ...
  if (plannedAmount > 0.0001) {
    ...
    spoolProgressHeader = 'Yarn Feed Progress';
    spoolProgressToneState = progressTone;
  } else if (fedAmount > 0.0001) {
    spoolProgress = 0;
    progressText =
      'Spool progress: Feeding yarn without a declared target — progress ring paused until planner target provided.';
    progressTone = 'warning';
    spoolProgressHeader = 'Yarn Feed';
    spoolProgressToneState = progressTone;
  } else {
    spoolProgress = 0;
  ...
  applySpoolProgressTone(spoolProgressToneState);
  ...
  if (yarnFlowTimingElement) {
    const preview =
      ...
      typeof patternPreviewGroup.userData === 'object'
        ...
        : null;
    const countdowns = computeFeedCountdowns(
      preview,
      yarnFeedStepIndices,
      progressRatio,
      previewDuration,
  ...
  cableChainNextFeedSeconds = nextFeedCountdown ? nextFeedCountdown.delta : null;
  cableChainFollowingFeedSeconds = followingFeedCountdown
    ? followingFeedCountdown.delta
    : null;
  cableChainRemainingFeeds =
    typeof remainingFeedCount === 'number' ? remainingFeedCount : null;
  cableChainCycleTimingDetail = cycleTimingDetail;
  updateCableChainBillboard({
    nextSeconds: cableChainNextFeedSeconds,
    followingSeconds: cableChainFollowingFeedSeconds,
    remainingFeeds: cableChainRemainingFeeds,
    cycleTimingDetail: cableChainCycleTimingDetail,
    extrusionActive: yarnExtrusionActive,
  });
  ...
  if (yarnFlowCalibrationElement) {
    ...
    if (Array.isArray(tensionCalibration) && tensionCalibration.length >= 2) {
      const minReading = tensionCalibration[0].reading;
      const maxReading = tensionCalibration[tensionCalibration.length - 1].reading;
      const minGrams = tensionCalibration[0].grams;
      const maxGrams = tensionCalibration[tensionCalibration.length - 1].grams;

      calibrationLabel =
        `Calibration: ${minReading.toFixed(1)}–${maxReading.toFixed(1)} reading → ` +
        `${minGrams.toFixed(1)}–${maxGrams.toFixed(1)} g.`;
      calibrationTone = 'info';

      if (tensionReading !== null) {
        if (tensionReading < minReading) {
          calibrationLabel += ' Current reading below range — clamping to sensor floor.';
          calibrationTone = 'warning';
        } else if (tensionReading > maxReading) {
          calibrationLabel += ' Current reading above range — clamping to sensor ceiling.';
          calibrationTone = 'warning';
        } else {
          calibrationLabel +=
            ` Current reading ${tensionReading.toFixed(1)} within calibrated span.`;
          calibrationTone = yarnExtrusionActive ? 'ready' : 'info';
        }
      }
    }
  ...
  if (yarnFlowTensionElement || yarnFlowRateElement) {
    const previousEvent =
      ...
      stepIndex > 0 &&
      Array.isArray(patternPlannerEvents) &&
      patternPlannerEvents.length > 0
        ? patternPlannerEvents[Math.min(stepIndex - 1, patternPlannerEvents.length - 1)]
        : null;

    const previousExtrusion = coerceFiniteNumber(previousEvent?.extrusion);
    const currentExtrusion = coerceFiniteNumber(event.extrusion);
    const stepDurationSeconds =
      Number.isFinite(patternPreviewDurationSeconds) &&
      Number.isFinite(totalSteps) &&
      totalSteps > 0
        ? patternPreviewDurationSeconds / totalSteps
        : null;

    if (currentExtrusion !== null && stepDurationSeconds !== null) {
      const baselineExtrusion = Number.isFinite(patternExtrusionBaseline)
        ? patternExtrusionBaseline
        : 0;
      const previousSample =
        previousExtrusion !== null ? previousExtrusion : baselineExtrusion;
      const extrusionDelta = Math.max(currentExtrusion - previousSample, 0);
      feedRateMmPerSec = extrusionDelta / stepDurationSeconds;
      feedRateLabel = `Feed rate: ${feedRateMmPerSec.toFixed(1)} mm/s`;
      feedRateTone = yarnExtrusionActive ? 'ready' : 'info';
    } else {
      feedRateLabel = yarnFlowRateFallbackMessage;
      feedRateTone = 'neutral';
    }
  }
  ...
  if (yarnFlowTensionElement) {
    ...
    if (tensionGrams !== null) {
      tensionLabel = `Tension telemetry: ${tensionGrams.toFixed(1)} g (sensor feed).`;
      tensionTone = yarnExtrusionActive ? 'ready' : 'info';
    } else if (sensorEstimate) {
      const suffix = sensorEstimate.clamped
        ? ` — clamped to ${sensorEstimate.min.toFixed(1)}–${sensorEstimate.max.toFixed(1)} reading range`
        : '';
      tensionLabel =
        `Tension telemetry: ${sensorEstimate.grams.toFixed(1)} g (hall sensor${suffix}).`;
      tensionTone = yarnExtrusionActive ? 'ready' : 'info';
    } else if (tensionReading !== null) {
      if (feedRateMmPerSec !== null) {
        tensionLabel =
          `Tension telemetry: Uncalibrated hall sensor (${tensionReading.toFixed(
            1,
          )} reading) — falling back to feed-rate estimate ${feedRateMmPerSec.toFixed(1)} mm/s.`;
        tensionTone = 'info';
      } else {
        tensionLabel =
          'Tension telemetry: Include tension_sensor_calibration to convert hall sensor readings.';
        tensionTone = 'warning';
      }
    } else if (feedRateMmPerSec !== null) {
      // Tension estimation calibrated to documented profiles:
      // - Lace: 20g @ 45mm/s
      // - Super Bulky: 95g @ 28mm/s
      // Linear interpolation between these points:
      const MIN_FEED_RATE = 45; // mm/s (lace)
      const MAX_FEED_RATE = 28; // mm/s (super bulky)
      const MIN_TENSION = 20; // g
      const MAX_TENSION = 95; // g
      // Clamp feed rate to expected range
      const clampedFeedRate = Math.max(
        Math.min(feedRateMmPerSec, MIN_FEED_RATE),
        MAX_FEED_RATE,
      );
      // Interpolate tension: higher feed rate = lower tension
      const estimatedGrams =
        MIN_TENSION +
        ((MAX_TENSION - MIN_TENSION) * (MIN_FEED_RATE - clampedFeedRate)) /
          (MIN_FEED_RATE - MAX_FEED_RATE);

      tensionLabel =
        `Tension telemetry: ${estimatedGrams.toFixed(1)} g (feed-rate estimate).`;
      tensionTone = yarnExtrusionActive ? 'ready' : 'info';
    }
  ...
  if (yarnFlowPositionElement) {
    const positionSegments = [
      ...
      `Z ${formatCoordinate(interpolatedPosition?.z ?? event.z)} mm`,
      `E ${formatCoordinate(
  ...
  lastSpoolCycleTimingDetail = cycleTimingDetail;
  lastSpoolCountdownToneSource = spoolProgressToneState;
  lastSpoolCountdownExtrusionActive = yarnExtrusionActive;
  refreshSpoolCountdownLabel();
}
...
function eventsFromPlannerPayload(payload) {
  ...
  return commands.map((entry) => {
    ...
    return {
      ...
      tensionGrams: coerceFiniteNumber(
        state.tension_grams ?? state.tension_g ?? state.tension,
      ),
      tensionSensorReading: coerceFiniteNumber(
        state.tension_sensor_reading ?? state.sensor_reading ?? state.tension_sensor,
      ),
    };
  });
}
...
function applyPatternPlannerEvents(events, options = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(options, 'machineProfileBounds')) {
    ...
    machineProfileMissingAxes = Array.isArray(options.machineProfileMissingAxes)
      ? options.machineProfileMissingAxes.filter((axis) => typeof axis === 'string')
      : computeMissingMachineProfileAxes(options.machineProfileBounds);
    currentMachineProfile = Object.prototype.hasOwnProperty.call(options, 'machineProfile')
      ? options.machineProfile
      : currentMachineProfile;
    updateMachineProfileEnvelope(machineProfileBounds, machineProfileMissingAxes);
  }
  ...
  if (patternPlannerEvents.length === 0 && options.resetOverlay !== false) {
    ...
    patternPreviewDurationSeconds = defaultPatternPreviewDurationSeconds;
  } else {
    ...
    const durationOverride =
      Number.isFinite(options.previewDurationSeconds) && options.previewDurationSeconds > 0
        ? options.previewDurationSeconds
        : null;
    const metadataDuration = Number.isFinite(plannerMetadata?.durationSeconds) &&
      plannerMetadata.durationSeconds > 0
      ? plannerMetadata.durationSeconds
      : null;
    patternPreviewDurationSeconds =
      durationOverride ?? metadataDuration ?? defaultPatternPreviewDurationSeconds;
  }
  patternPreviewElapsed = 0;
  setPatternPreviewPaused(false);
  ...
  boundsComparison = comparePlannerToMachineBounds(plannerBounds, machineProfileBounds);
  ...
  if (patternPlannerEvents.length > 0) {
    ...
    updatePatternOverlay(0, 0, { segmentProgress: 0 });
    ...
    updateBoundsComparisonPanel(null, null);
    updateTravelEnvelope(null, null);
  }
}
...
function updateMachineProfilePanel(machineProfile) {
  ...
  currentMachineProfile = machineProfile && typeof machineProfile === 'object'
    ? machineProfile
    : null;
  machineProfileMissingAxes = computeMissingMachineProfileAxes(currentMachineProfile);
  machineProfileBounds = normalizeMachineProfileBounds(machineProfile);
  updateMachineProfileEnvelope(machineProfileBounds, machineProfileMissingAxes);
...
async function loadPlannerPreviewFromSource() {
  ...
  try {
    ...
    const machineProfile = payload?.machine_profile ?? null;
    const machineProfileMissingAxes = computeMissingMachineProfileAxes(machineProfile);
    const normalizedMachineProfileBounds =
      normalizeMachineProfileBounds(machineProfile);
    machineProfileBounds = normalizedMachineProfileBounds;
    const metadata = {
      ...(payload?.metadata && typeof payload.metadata === 'object' ? payload.metadata : {}),
      version: payload?.version ?? null,
      units: typeof payload?.units === 'string' ? payload.units : null,
      fileSizeBytes: Number(response.headers.get('Content-Length')) || null,
      fileName: deriveFileNameFromSource(patternPreviewSource) || 'base_chain_row.planner.json',
    };
    if (!metadata.source) {
      metadata.source = 'Base chain row asset';
    }
    applyPatternPlannerEvents(events, {
      defaults,
      bounds,
      metadata,
      machineProfileBounds: machineProfile,
      machineProfileMissingAxes,
      machineProfile,
    });
...
function buildPlannerPreview(events, options = {}) {
  ...
  let minZ = coerceFiniteNumber(events[0]?.z ?? 0) ?? 0;
  let maxZ = minZ;
  ...
  if (Number.isFinite(minZ) && Number.isFinite(maxZ) && maxZ >= minZ) {
    fallbackBounds.z = { min: minZ, max: maxZ };
  }
  ...
  const displayZMin = Number.isFinite(computedBounds?.z?.min)
    ? computedBounds.z.min
    : Number.isFinite(minZ)
      ? minZ
      : 0;
  const displayZMax = Number.isFinite(computedBounds?.z?.max)
    ? computedBounds.z.max
  ...
  const boundsCage = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.BoxGeometry(usableWidth, usableHeight, usableDepth)),
  ...
  const zCenterRatio = ((displayZMin + displayZMax) / 2 - displayZMin) / zSpan;
  const normalizedZCenter = zCenterRatio - 0.5;
  boundsCage.position.set(0, 0, normalizedZCenter * usableDepth);
  boundsCage.name = 'planner-bounds-cage';
  boundsGroup.add(boundsCage);
  addHoverInfo(
    boundsCage,
    'Mint bounds cage — tracks the planner Z span above the preview plane.',
    { passthrough: true },
  );
  boundsCageControllers.push({
    material: boundsCage.material,
    baseOpacity: boundsCage.material.opacity ?? 0.62,
    opacityAmplitude: 0.18,
    speed: 1.4,
    phaseOffset: 0.4,
  });

  const zSpanColumnMaterial = new THREE.MeshStandardMaterial({
    color: 0x5fffd2,
    emissive: 0x2cf9c5,
  ...
  const zSpanColumn = new THREE.Mesh(
    new THREE.CylinderGeometry(0.013, 0.013, usableDepth, 24),
    zSpanColumnMaterial,
  );
  zSpanColumn.rotation.x = Math.PI / 2;
  zSpanColumn.position.set(-(usableWidth / 2) - 0.055, 0, normalizedZCenter * usableDepth);
  zSpanColumn.renderOrder = 3;
  boundsGroup.add(zSpanColumn);
  addHoverInfo(
    zSpanColumn,
    'Z-span beacon — mint column spotlights the planner clearance band.',
    { passthrough: true },
  );
  ...
  const zMarkerGeometry = new THREE.TorusGeometry(0.08, 0.01, 16, 48);
  const zMinMarker = new THREE.Mesh(zMarkerGeometry, markerMaterial.clone());
  zMinMarker.rotation.x = Math.PI / 2;
  zMinMarker.position.set(0, 0, -usableDepth / 2);
  addHoverInfo(
    zMinMarker,
    'Z-span markers — twin halos pulse at the planner min and max heights.',
    { passthrough: true },
  );
  zSpanColumn.add(zMinMarker);
  const zMaxMarker = new THREE.Mesh(zMarkerGeometry, markerMaterial.clone());
  zMaxMarker.rotation.x = Math.PI / 2;
  zMaxMarker.position.set(0, 0, usableDepth / 2);
  zSpanColumn.add(zMaxMarker);
  addHoverInfo(
    zMaxMarker,
    'Z-span markers — twin halos pulse at the planner min and max heights.',
    { passthrough: true },
  ...
  boundsZMarkerControllers.push(
    {
      mesh: zSpanColumn,
      material: zSpanColumnMaterial,
      baseOpacity: zSpanColumnMaterial.opacity ?? 0.36,
      opacityAmplitude: 0.16,
      baseIntensity: zSpanColumnMaterial.emissiveIntensity ?? 0.8,
      intensityAmplitude: 0.5,
      speed: 1.8,
      phaseOffset: 0.2,
    },
    {
      mesh: zMinMarker,
      material: zMinMarker.material,
      baseOpacity: zMinMarker.material.opacity ?? 0.72,
      opacityAmplitude: 0.18,
      baseIntensity: zMinMarker.material.emissiveIntensity ?? 0.9,
      intensityAmplitude: 0.6,
      scaleAmplitude: 0.16,
      speed: 2.4,
      phaseOffset: 0.1,
    },
    {
      mesh: zMaxMarker,
      material: zMaxMarker.material,
      baseOpacity: zMaxMarker.material.opacity ?? 0.72,
      opacityAmplitude: 0.18,
      baseIntensity: zMaxMarker.material.emissiveIntensity ?? 0.9,
      intensityAmplitude: 0.6,
      scaleAmplitude: 0.16,
      speed: 2.2,
      phaseOffset: 1.2,
  ...
  const profileBounds = options.machineProfileBounds;
  if (profileBounds?.x && profileBounds?.y) {
    const profileXSpan = Math.max(profileBounds.x.max - profileBounds.x.min, 0.0001);
    const profileYSpan = Math.max(profileBounds.y.max - profileBounds.y.min, 0.0001);
    const profileWidth = THREE.MathUtils.clamp(profileXSpan / xSpan, 0, 1) * usableWidth;
    const profileHeight = THREE.MathUtils.clamp(profileYSpan / ySpan, 0, 1) * usableHeight;

    const profileHasMissingAxes = Array.isArray(options.machineProfileMissingAxes)
      ? options.machineProfileMissingAxes.length > 0
      : false;
    const profileIsSafe =
      !(options.boundsComparison?.exceedingAxes?.length) && !profileHasMissingAxes;
    const outlineColor = profileIsSafe ? 0x5fffd2 : 0xffc27d;
    const fillColor = profileIsSafe ? 0x2cf9c5 : 0xffb173;
    const fillOpacity = profileIsSafe ? 0.08 : 0.14;

    const centerXRatio =
      ((profileBounds.x.min + profileBounds.x.max) / 2 - displayXMin) / xSpan - 0.5;
    const centerYRatio =
      ((profileBounds.y.min + profileBounds.y.max) / 2 - displayYMin) / ySpan - 0.5;
    const offsetX = THREE.MathUtils.clamp(centerXRatio, -0.75, 0.75) * usableWidth;
    const offsetY = THREE.MathUtils.clamp(centerYRatio, -0.75, 0.75) * usableHeight;
    ...
    const profileMaterial = new THREE.LineDashedMaterial({
      color: outlineColor,
      linewidth: 1,
      scale: 1,
      dashSize: 0.05,
    ...
    const profileOutline = new THREE.LineLoop(profileGeometry, profileMaterial);
    profileOutline.computeLineDistances();
    profileOutline.position.set(offsetX, offsetY, -0.001);
    profileOutline.renderOrder = 3;

    const profileFill = new THREE.Mesh(
      new THREE.PlaneGeometry(profileWidth, profileHeight),
    ...
    profileFill.position.set(offsetX, offsetY, -0.0015);
    profileFill.renderOrder = 2;
    boundsGroup.add(profileFill);
    boundsGroup.add(profileOutline);
    addHoverInfo(profileFill, buildMachineProfileEnvelopeText(profileBounds), {
      passthrough: true,
    });
  }
...
function interpolatePlannerEventCoordinates(events, index, progress) {
  ...
  const start = events?.[safeIndex] ?? {};
  const end = events?.[Math.min(safeIndex + 1, (events?.length ?? 1) - 1)] ?? start;
  ...
  const blend = (a, b) => {
    const startValue = Number.isFinite(a) ? a : null;
    const endValue = Number.isFinite(b) ? b : null;
  ...
  return {
    ...
    z: blend(start.z, end.z),
    extrusion: blend(start.extrusion, end.extrusion),
  };
}
...
function updatePatternOverlay(stepIndex, progressRatio, options = {}) {
  ...
  const interpolatedPosition = interpolatePlannerEventCoordinates(
    patternPlannerEvents,
    clampedIndex,
    options.segmentProgress ?? 0,
  );
  ...
  const shouldUpdatePosition =
    clampedIndex !== lastPatternStepIndex || progressChanged;
...
function updatePatternPreview(elapsedSeconds) {
  ...
  updatePatternOverlay(index, progress, { segmentProgress });
}
...
function createSafeAccessPath() {
  ...
  footstepControllers.push({
    footprint,
    toe,
    baseHeight: footprint.position.y,
    baseToeHeight: toe.position.y,
    liftAmplitude: 0.05,
    speed: 2.1,
    phaseOffset,
  });
...
function createTravelEnvelopeVolumes({ center, bedSize }) {
  const group = new THREE.Group();
  group.name = 'travel-envelope-volumes';
  group.position.copy(center.clone().add(new THREE.Vector3(0, 0.08, 0)));

  const baseBoxGeometry = new THREE.BoxGeometry(1, 1, 1);
  const edgeGeometry = new THREE.EdgesGeometry(baseBoxGeometry);

  const plannerOutlineMaterial = new THREE.LineDashedMaterial({
    color: 0x9ee5ff,
    linewidth: 1,
    dashSize: 0.18,
    gapSize: 0.1,
  ...
  const plannerOutline = new THREE.LineSegments(edgeGeometry, plannerOutlineMaterial);
  plannerOutline.computeLineDistances();
  plannerOutline.name = 'planner-travel-envelope-cage';
  group.add(plannerOutline);

  const machineOutlineMaterial = new THREE.LineDashedMaterial({
    color: 0x5fffd2,
    linewidth: 1,
    dashSize: 0.16,
    gapSize: 0.12,
  ...
  const machineOutline = new THREE.LineSegments(edgeGeometry.clone(), machineOutlineMaterial);
  machineOutline.computeLineDistances();
  machineOutline.name = 'machine-profile-travel-envelope-cage';
  group.add(machineOutline);

  const plannerFill = new THREE.Mesh(
    baseBoxGeometry.clone(),
    new THREE.MeshBasicMaterial({
      color: 0x4fb2ff,
      transparent: true,
      opacity: 0.08,
  ...
  plannerFill.name = 'planner-travel-envelope-fill';
  group.add(plannerFill);

  const machineFill = new THREE.Mesh(
    baseBoxGeometry.clone(),
    new THREE.MeshBasicMaterial({
      color: 0x2cf9c5,
      transparent: true,
  ...
  machineFill.name = 'machine-profile-travel-envelope-fill';
  group.add(machineFill);

  const plannerLabel = buildLabel('Planner travel envelope', '#d9f4ff');
  plannerLabel.position.set(0, 0.7, 0);
  plannerLabel.scale.multiplyScalar(0.32);
  group.add(plannerLabel);

  const machineLabel = buildLabel('Machine profile travel envelope', '#ccfff0');
  machineLabel.position.set(0, 0.82, 0);
  machineLabel.scale.multiplyScalar(0.32);
  group.add(machineLabel);

  const zHaloMaterial = new THREE.MeshStandardMaterial({
    color: 0x9ee5ff,
    emissive: 0x69f9ff,
    emissiveIntensity: 0.9,
    metalness: 0.35,
  ...
  const zHaloGeometry = new THREE.RingGeometry(0.42, 0.56, 72, 1);
  const zHaloGroup = new THREE.Group();
  zHaloGroup.name = 'planner-z-span-halos';
  zHaloGroup.visible = false;

  const minZHalo = new THREE.Mesh(zHaloGeometry, zHaloMaterial.clone());
  minZHalo.rotation.x = -Math.PI / 2;
  zHaloGroup.add(minZHalo);
  addHoverInfo(
    minZHalo,
    'Planner Z-span halos — rings mark the min and max Z heights for the planner envelope.',
    { passthrough: true },
  );

  const maxZHalo = new THREE.Mesh(zHaloGeometry, zHaloMaterial.clone());
  maxZHalo.rotation.x = -Math.PI / 2;
  zHaloGroup.add(maxZHalo);
  addHoverInfo(
    maxZHalo,
    'Planner Z-span halos — rings mark the min and max Z heights for the planner envelope.',
    { passthrough: true },
  ...
  group.add(zHaloGroup);

  addHoverInfo(
    plannerOutline,
    'Planner travel envelope cage — projects planner bounds above the gantry for quick checks.',
    { passthrough: true },
  );
  addHoverInfo(
    plannerFill,
    'Planner travel volume — flashes amber when bounds exceed the machine profile.',
    { passthrough: true },
  );
  addHoverInfo(
    machineFill,
    'Machine profile travel envelope — mirrors travel_min_mm and travel_max_mm from the machine profile. Travel envelope cage stays aligned to the gantry bed.',
    { passthrough: true },
  );

  boundsCageControllers.push(
    {
      material: plannerOutlineMaterial,
      baseOpacity: plannerOutlineMaterial.opacity ?? 0.86,
      opacityAmplitude: 0.14,
      speed: 1.5,
      phaseOffset: 0.35,
    },
    {
      material: plannerFill.material,
      getBaseOpacity: () => plannerFill.material.opacity ?? 0.08,
      opacityAmplitude: 0.06,
      speed: 1.4,
      phaseOffset: 0.22,
    },
    {
      material: machineOutlineMaterial,
      baseOpacity: machineOutlineMaterial.opacity ?? 0.82,
      opacityAmplitude: 0.12,
      speed: 1.2,
      phaseOffset: 0.18,
    },
    {
      material: machineFill.material,
      baseOpacity: machineFill.material.opacity ?? 0.12,
      opacityAmplitude: 0.06,
      speed: 1.8,
      phaseOffset: 0.42,
    },
  ...
  travelEnvelopeHaloControllers.push(
    {
      mesh: minZHalo,
      material: minZHalo.material,
      baseOpacity: minZHalo.material.opacity ?? 0.78,
      opacityAmplitude: 0.16,
      baseIntensity: minZHalo.material.emissiveIntensity ?? 0.9,
      intensityAmplitude: 0.55,
      scaleAmplitude: 0.18,
      speed: 2.0,
      phaseOffset: 0.3,
    },
    {
      mesh: maxZHalo,
      material: maxZHalo.material,
      baseOpacity: maxZHalo.material.opacity ?? 0.78,
      opacityAmplitude: 0.16,
      baseIntensity: maxZHalo.material.emissiveIntensity ?? 0.9,
      intensityAmplitude: 0.55,
      scaleAmplitude: 0.22,
      speed: 2.3,
      phaseOffset: 1.2,
  ...
  const fallbackSpan = {
    x: Math.max(bedSize.width / mmToScene, 1),
    y: Math.max(bedSize.depth / mmToScene, 1),
    z: 120,
  };

  plannerOutline.visible = false;
  plannerFill.visible = false;
  plannerLabel.visible = false;
  machineOutline.visible = false;
  machineFill.visible = false;
...
function createElectronicsBay() {
  ...
  const controllerStack = new THREE.Group();
  controllerStack.position.set(0.1, 0.26, 0.08);
  group.add(controllerStack);
  ...
  pcb.position.set(0, 0.06, 0);
  pcb.castShadow = true;
  controllerStack.add(pcb);
  addHoverInfo(
    pcb,
    'Controller stack PCB — SKR Mini footprint exposed through the tinted access panel.',
    { passthrough: true },
  );

  const driverColors = [0x4c6fff, 0x6cb4ff, 0x5ad6ff];
  driverColors.forEach((color, index) => {
    ...
    driver.position.set(-0.22 + index * 0.28, 0.18, -0.05 + (index % 2) * 0.12);
    driver.castShadow = true;
    controllerStack.add(driver);
    controllerStackGlowControllers.push({
      material: driver.material,
      restIntensity: 0.42,
      activeIntensity: 1.1,
      idlePulseStrength: 0.3,
      pulseSpeed: 2.0 + index * 0.3,
      phaseOffset: index * 0.8,
      restOpacity: driver.material.opacity,
      activeOpacity: 1.0,
      damping: 5.8,
    });
  });
  ...
  mcu.position.set(0.36, 0.16, 0.12);
  mcu.castShadow = true;
  controllerStack.add(mcu);
  addHoverInfo(
    mcu,
    'SKR Mini control SoC — routes planner pulses to the stepper drivers.',
  );
  ...
  controllerStack.add(heatsink);

  const statusLedMaterial = new THREE.MeshStandardMaterial({
    color: 0x7dffbe,
    emissive: 0x33d18a,
    emissiveIntensity: 0.9,
    metalness: 0.2,
  ...
  const pcbLabel = buildLabel('Controller Stack', '#cde8ff');
  pcbLabel.position.set(0.12, 0.82, -0.18);
  pcbLabel.scale.multiplyScalar(0.36);
  controllerStack.add(pcbLabel);
  ...
  fanBlades.name = 'electronics-bay-exhaust-blades';
  fanBlades.position.copy(fan.position.clone().add(new THREE.Vector3(0, 0, 0.07)));
  ...
  const fanBladeGeometry = new THREE.BoxGeometry(0.04, 0.46, 0.12);
  for (let index = 0; index < 3; index += 1) {
    const blade = new THREE.Mesh(fanBladeGeometry, fanBladeMaterial);
    blade.rotation.z = (Math.PI / 3) * index;
  ...
  addHoverInfo(
    fanBlades,
    'Electronics bay exhaust blades — idle softly and sprint with yarn feed pulses.',
  );
  ...
  thermistorConduitControllers.push({
    material: thermistorMaterial,
    light: thermistorGlow,
    baseIntensity: 0.55,
    activeIntensity: 1.4,
    baseOpacity: 0.82,
    activeOpacity: 1.0,
    baseLight: thermistorGlow.intensity,
    activeLight: 1.6,
    shimmerSpeed: 2.4,
    countdownBias: 0.6,
    damping: 4.8,
    defaults: {
      color: thermistorMaterial.color.getHex(),
      emissive: thermistorMaterial.emissive.getHex(),
    },
  });
  ...
  const statusLight = new THREE.PointLight(0x66ffc2, 1.0, 2.2);
  ...
  addHoverInfo(
    statusLed,
    'Status LED — breathes at idle and brightens with yarn feed pulses.',
    { passthrough: true },
  );
  statusIndicatorControllers.push({
    material: statusLed.material,
    light: statusLight,
    idleIntensity: 0.85,
    activeIntensity: 1.6,
    idleLight: 1.0,
...
function createCableChain() {
  ...
  addHoverInfo(
    chain,
    'Cable chain — routes gantry wiring to the hook carriage with a chasing glow.',
  );
  ...
  const chaseOrb = new THREE.Mesh(new THREE.SphereGeometry(0.09, 20, 20), chaseMaterial);
  chainCurve.getPointAt(0.12, chaseOrb.position);
  chaseOrb.castShadow = false;
  chaseOrb.receiveShadow = false;
  group.add(chaseOrb);
  addHoverInfo(
    chaseOrb,
  ...
  const chaseLight = new THREE.PointLight(0x7cc2ff, 1.0, 3.0);
  chaseLight.position.copy(chaseOrb.position);
  group.add(chaseLight);

  const cableChainBillboard = createCableChainBillboard();
  if (cableChainBillboard) {
    cableChainBillboardController = cableChainBillboard;
    const { sprite } = cableChainBillboard;
    const anchorPoint = chainCurve.getPointAt(0.22);
    sprite.position.copy(anchorPoint.clone().add(new THREE.Vector3(0, 0.42, 0)));
    sprite.scale.multiplyScalar(0.86);
    group.add(sprite);
    addHoverInfo(
      sprite,
      'Cable chain countdown — displays feed timing and remaining pulses near the chase orb.',
    );
  }

  cableChainChaseControllers.push({
    mesh: chaseOrb,
    light: chaseLight,
    ...
    restLight: chaseLight.intensity,
    activeLight: 1.6,
    damping: 6.0,
  });
  cableChainPulseControllers.push({
...
function createV1CAssembly() {
  ...
  const insertGrid = new THREE.Group();
  insertGrid.name = 'heat-set-insert-grid';
  ...
  const insertGlowMaterial = new THREE.MeshStandardMaterial({
    color: 0x86f5ff,
    emissive: 0x86f5ff,
  ...
  // Dimensions tuned to match M3 brass insert proportions in scene units (1 unit ≈ 10 cm).
  const insertGeometry = new THREE.CylinderGeometry(0.055, 0.07, 0.06, 24); // radiusTop, radiusBottom, height
  const insertGlowGeometry = new THREE.CylinderGeometry(0.11, 0.11, 0.012, 24); // slightly oversized halo disk
  const insertSpacingX = 0.8; // M3 insert center-to-center spacing in the X direction (~8 cm)
  const insertSpacingZ = 0.75; // M3 insert center-to-center spacing in the Z direction (~7.5 cm)
  const insertRows = 4;
  const insertCols = 5;
  for (let row = 0; row < insertRows; row += 1) {
    for (let col = 0; col < insertCols; col += 1) {
      const x = insertSpacingX * (col - (insertCols - 1) / 2);
      const z = insertSpacingZ * (row - (insertRows - 1) / 2);
      const insert = new THREE.Mesh(insertGeometry, insertMaterial.clone());
      insert.position.set(x, bedTopY + 0.035, z); // small lift above the bed to match insert head height
      insert.castShadow = true;
      insertGrid.add(insert);

      const glow = new THREE.Mesh(insertGlowGeometry, insertGlowMaterial.clone());
      glow.position.set(x, bedTopY + 0.07, z);
      glow.rotation.x = Math.PI / 2;
      glow.renderOrder = 3;
      insertGrid.add(glow);

      insertGridControllers.push({
        ...
        opacityAmplitude: 0.25,
        speed: 0.85,
        // 0.18 rad per index offsets the sine wave by ~10° per insert so the ripple walks diagonally across the grid.
        phaseOffset: (row * insertCols + col) * 0.18,
      });
    }
  }
  insertGrid.position.copy(bed.position);
  group.add(insertGrid);
  addHoverInfo(
    insertGrid,
    'Heat-set insert grid — glowing M3 anchors map the spacing for accessory plates and fixtures.',
    {
      label: 'Insert Grid',
      panelTone: 'info',
    },
  ...
  const travelEnvelope = createTravelEnvelopeVolumes({
    center: new THREE.Vector3(bed.position.x, bedTopY, bed.position.z),
    bedSize: { width: 4.8, depth: 3.2 },
  });
  travelEnvelopeController = travelEnvelope;
  group.add(travelEnvelope.group);
  ...
  anchorOffsets.forEach(([x, z], index) => {
    ...
    anchorPulseControllers.push({
      ...
      lightAmplitude: 0.75,
      sequenceIndex: index,
      sequenceCount: anchorOffsets.length,
      sequenceDuration: 9.5,
    });
  ...
  addHoverInfo(
    anchorsGroup,
    'Anchor swap order sweep — rear-left puck kicks off the clockwise halo so swaps match the floor choreography.',
    { passthrough: true },
  );
  ...
  zLiftGlowControllers.push({
    material: leadscrewMaterial,
    baseIntensity: leadscrewMaterial.emissiveIntensity ?? 0.35,
    amplitude: 0.35,
    lightAmplitude: 0.25,
    speed: 1.4,
    phaseOffset: 0.15,
  ...
  zLiftGlowControllers.push({
    material: nutMaterial,
    baseIntensity: nutMaterial.emissiveIntensity ?? 0.5,
    amplitude: 0.4,
    lightAmplitude: 0.3,
    speed: 1.9,
    phaseOffset: 0.65,
  ...
  zLiftGlowControllers.push({
    material: flexureMaterial,
    baseIntensity: flexureMaterial.emissiveIntensity ?? 0.45,
    amplitude: 0.35,
    lightAmplitude: 0.25,
    speed: 2.3,
    phaseOffset: 0.35,
  ...
  zLiftGlowControllers.push({
    light: zLiftGlow,
    baseLightIntensity: zLiftGlow.intensity,
    lightAmplitude: 0.35,
    speed: 1.3,
    phaseOffset: 0.9,
    feedBoost: 0.25,
  ...
  const spoolCountdownLabel = createSpoolCountdownLabel();
  if (spoolCountdownLabel) {
    spoolCountdownLabelController = spoolCountdownLabel;
    const { sprite } = spoolCountdownLabel;
    sprite.position.set(0, 2.18, 0);
    spoolProgressGroup.add(sprite);
    addHoverInfo(
      sprite,
      'Countdown ribbon — keeps the next two feed pulses pinned above the spool '
        + 'progress display.',
    );
  }
  addHoverInfo(
    ...
    'Spool progress ring — fills as the planner feeds yarn around the supply reel. ' +
      'A pre-pulse sweep wraps the ring as feed countdowns approach while the inner ' +
      'cycle clock marks preview loop progress.',
    { passthrough: true },
  );
  ...
  const spoolCycleSegmentCount = 18;
  const spoolCycleAngle = (Math.PI * 2) / spoolCycleSegmentCount;
  for (let index = 0; index < spoolCycleSegmentCount; index += 1) {
    const segmentMaterial = new THREE.MeshStandardMaterial({
      color: 0x9de2ff,
      emissive: 0x4a9cff,
    ...
    const segment = new THREE.Mesh(
      new THREE.TorusGeometry(0.78, 0.028, 10, 14, spoolCycleAngle),
      segmentMaterial,
    );
    segment.rotation.y = Math.PI / 2;
    segment.rotation.x = index * spoolCycleAngle;
    spoolProgressGroup.add(segment);
    spoolCycleSegments.push({
      mesh: segment,
      material: segmentMaterial,
      baseIntensity: 0.08,
      activeIntensity: 0.8,
  ...
  const filamentSensor = new THREE.Group();
  filamentSensor.name = 'filament-break-sensor';
  filamentSensor.position.set(-1.76, 1.92, -1.56);

  const filamentBracket = new THREE.Mesh(
    new THREE.BoxGeometry(0.36, 0.18, 0.32),
    new THREE.MeshStandardMaterial({
      color: 0x273141,
      metalness: 0.42,
      roughness: 0.4,
  ...
  filamentBracket.castShadow = true;
  filamentBracket.position.set(0, -0.04, 0);
  filamentSensor.add(filamentBracket);

  const filamentWindowMaterial = new THREE.MeshStandardMaterial({
    color: 0xffe3b5,
    metalness: 0.2,
    roughness: 0.2,
    emissive: 0x3a2a0d,
  ...
  const filamentWindow = new THREE.Mesh(
    new THREE.BoxGeometry(0.22, 0.22, 0.22),
    filamentWindowMaterial,
  );
  filamentWindow.castShadow = true;
  filamentSensor.add(filamentWindow);

  const filamentIndicator = new THREE.PointLight(0xffb86c, 0.9, 2.2);
  filamentIndicator.position.set(0, 0.12, 0);
  filamentSensor.add(filamentIndicator);

  const filamentGuide = new THREE.Object3D();
  filamentGuide.position.set(0, 0.16, 0);
  filamentSensor.add(filamentGuide);
  filamentSensor.userData.guide = filamentGuide;

  const filamentLabel = buildLabel('Filament Break Sensor', '#ffe7c7');
  filamentLabel.position.set(0.02, 0.74, 0);
  filamentLabel.scale.multiplyScalar(0.42);
  filamentSensor.add(filamentLabel);

  addHoverInfo(
    filamentSensor,
    'Filament break sensor — optional inline detector to pause runs if yarn snaps.',
  );
  filamentSensorControllers.push({
    material: filamentWindowMaterial,
    light: filamentIndicator,
    idleIntensity: 0.35,
    activeIntensity: 1.2,
    idleLight: filamentIndicator.intensity,
    activeLight: 2.0,
    linkedToExtrusion: true,
    damping: 4.4,
  });

  group.add(filamentSensor);
  ...
  const filamentSensorPoint = new THREE.Vector3();
  filamentSensor.userData.guide.getWorldPosition(filamentSensorPoint);
  ...
  const ptfeCurve = new THREE.CatmullRomCurve3([
    ...
    tensionEntryPoint,
    tensionExitPoint,
    filamentSensorPoint.clone().add(new THREE.Vector3(0.1, 0.08, -0.02)),
    filamentSensorPoint,
...
function createTensionLabDisplay() {
  ...
  const automationPath = new THREE.CatmullRomCurve3([
    new THREE.Vector3(-0.95, 0.28, -0.42),
    new THREE.Vector3(-0.2, 0.32, -0.22),
    new THREE.Vector3(0.65, 0.34, 0.12),
  ]);
  const automationPoints = automationPath.getPoints(40);
  const automationGeometry = new THREE.BufferGeometry().setFromPoints(automationPoints);
  const automationMaterial = new THREE.LineDashedMaterial({
    color: 0x8fffe2,
    linewidth: 1,
    scale: 1,
    dashSize: 0.08,
  ...
  const automationLine = new THREE.Line(automationGeometry, automationMaterial);
  automationLine.computeLineDistances();
  automationLine.position.y = 0.02;
  automationLine.renderOrder = 6;
  group.add(automationLine);
  addHoverInfo(
    automationLine,
    'Automation sweep — traces the load-cell carriage path across the calibration rail and '
      + 'accelerates as yarn feed pulses approach.',
    { passthrough: true },
  );
  automationSweepControllers.push({
    material: automationMaterial,
    idleSpeed: 0.18,
    approachSpeed: 0.34,
    activeSpeed: 0.6,
    baseOpacity: automationMaterial.opacity ?? 0.85,
    activeOpacity: 1.0,
    damping: 4.2,
  });

  const automationTracer = new THREE.Mesh(
  ...
  automationTracer.position.copy(automationPath.getPoint(0));
  automationTracer.castShadow = true;
  automationTracer.receiveShadow = true;
  group.add(automationTracer);

  const automationLight = new THREE.PointLight(0x9bffe8, 0.9, 2.6);
  automationLight.position.copy(automationTracer.position.clone().add(new THREE.Vector3(0, 0.18, 0)));
  group.add(automationLight);

  tensionLabPathControllers.push({
    tracer: automationTracer,
    light: automationLight,
    curve: automationPath,
    travel: 0,
    activeSpeed: 0.24,
    idleSpeed: 0.06,
    restIntensity: 0.4,
    ...
    restOpacity: 0.4,
    activeOpacity: 0.95,
    restLightIntensity: automationLight.intensity,
    activeLightIntensity: 1.6,
    damping: 5.5,
  });
  ...
  addHoverInfo(
    servoIndicator,
    'Servo status orb — brightens with yarn feed pulses to mirror the horn sweep.',
  );
  ...
  servoSignalControllers.push({
    ...
    restIntensity: 0.25,
    activeIntensity: 1.2,
    restLightIntensity: servoLight.intensity,
    activeLightIntensity: 1.8,
    restOpacity: 0.35,
    activeOpacity: 0.9,
    damping: 5.5,
  });
...
function createCalibrationLabDisplay() {
  const group = new THREE.Group();
  group.name = 'calibration-lab-display';
  ...
  group.add(deck);
  addHoverInfo(deck, 'Calibration bench — tunes sensors before production runs.');
  ...
  const riser = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.32, 0.8), riserMaterial);
  riser.position.set(-0.16, 0.42, 0);
  riser.castShadow = true;
  group.add(riser);
  addHoverInfo(
    riser,
    'Sensor staging block — seats hall sensors and wiring harnesses for tuning.',
  );

  const loadCell = new THREE.Mesh(
    new THREE.BoxGeometry(0.48, 0.12, 0.32),
    new THREE.MeshStandardMaterial({
      color: 0xffd9a3,
      emissive: 0x8c5b15,
      emissiveIntensity: 0.7,
  ...
  loadCell.position.set(0.6, 0.46, 0.12);
  loadCell.castShadow = true;
  group.add(loadCell);
  addHoverInfo(loadCell, 'Load cell mock — anchors pull-force probes for calibration.');

  const gauge = new THREE.Mesh(
    new THREE.CylinderGeometry(0.12, 0.12, 0.5, 20),
    new THREE.MeshStandardMaterial({
      color: 0x97e5ff,
      emissive: 0x2c7a9d,
      emissiveIntensity: 0.65,
  ...
  gauge.position.set(0.6, 0.8, 0.12);
  gauge.castShadow = true;
  group.add(gauge);

  const probe = new THREE.Mesh(
    new THREE.ConeGeometry(0.08, 0.32, 20),
    new THREE.MeshStandardMaterial({
      color: 0xfff7de,
      emissive: 0x8a7844,
      emissiveIntensity: 0.5,
  ...
  probe.position.set(0.6, 1.06, 0.12);
  ...
  sensorScreen.position.set(-0.6, 0.82, 0.06);
  sensorScreen.rotation.y = Math.PI / 12;
  sensorScreen.castShadow = true;
  group.add(sensorScreen);
  addHoverInfo(
    sensorScreen,
  ...
  const label = buildLabel('Calibration Lab', '#c7e7ff');
...
function populateProductLines() {
  ...
  const clusterDefinitions = [
    ...
    },
    {
      ...
      roadmap:
        'Verifies sensors, probes load cells, and trims offsets before production runs.',
      hoverText:
        'Calibration lab pedestal — validates hall sensors and load-cell fixtures.',
      position: new THREE.Vector3(2.6, 0, 6.8),
      accentColor: 0x92d6ff,
      hoverColor: 0x1b2d3f,
      buildDisplay: createCalibrationLabDisplay,
      displayOffset: 0.02,
    },
    {
      name: 'Pattern Studio',
...
function animate() {
  ...
  insertGridControllers.forEach((controller) => {
    const {
      insertMaterial,
      glowMaterial,
      baseIntensity = 0,
    ...
    const swing = Math.sin(elapsed * speed + phaseOffset);
    const intensity = Math.max(0, baseIntensity + intensityAmplitude * swing);
    if (insertMaterial && typeof insertMaterial.emissiveIntensity === 'number') {
      insertMaterial.emissiveIntensity = intensity;
    }
    if (
      glowMaterial &&
      ...
      const opacity = THREE.MathUtils.clamp(
        baseOpacity + opacityAmplitude * (0.5 + 0.5 * swing),
        0,
        1,
      );
      glowMaterial.opacity = opacity;
    }
    if (glowMaterial && typeof glowMaterial.emissiveIntensity === 'number') {
      glowMaterial.emissiveIntensity = Math.max(0, intensity * 1.2);
    }
  });
  footstepControllers.forEach((controller) => {
    const {
      footprint,
      toe,
      baseHeight,
    ...
    const cadence = Math.max(Math.sin(elapsed * speed + phaseOffset), 0);
    const lift = cadence * liftAmplitude;
    footprint.position.y = baseHeight + lift;
    toe.position.y = baseToeHeight + lift * 0.82;
  });
  boundsCageControllers.forEach((controller) => {
    const {
      material,
      baseOpacity = 0,
      getBaseOpacity,
    ...
    const swing = Math.sin(elapsed * speed + phaseOffset);
    const opacityBase =
      typeof getBaseOpacity === 'function' ? getBaseOpacity() : baseOpacity;
    const opacity = THREE.MathUtils.clamp(
      opacityBase + swing * opacityAmplitude,
      0,
      1,
    );
    material.opacity = opacity;
  });
  travelEnvelopeHaloControllers.forEach((controller) => {
    const {
      mesh,
      material,
      baseOpacity = 0,
    ...
    const swing = Math.sin(elapsed * speed + phaseOffset);
    const pulse = 0.5 + 0.5 * swing;

    if (material) {
      if (typeof material.opacity === 'number') {
        const opacity = THREE.MathUtils.clamp(
          baseOpacity + swing * opacityAmplitude,
          0,
          1,
        );
        material.opacity = opacity;
      ...
      if (typeof material.emissiveIntensity === 'number') {
        material.emissiveIntensity = Math.max(
          0,
          baseIntensity + intensityAmplitude * pulse,
        );
      }
    }

    if (mesh && scaleAmplitude) {
      const baseScale = mesh.userData.baseScale;
      if (!baseScale) {
        return;
      }
      const scale = 1 + scaleAmplitude * pulse;
      mesh.scale.set(baseScale.x * scale, baseScale.y, baseScale.z * scale);
    }
  });
  boundsZMarkerControllers.forEach((controller) => {
    const {
      mesh,
      material,
      baseOpacity = 0,
    ...
    if (material) {
      ...
      if (typeof material.emissiveIntensity === 'number') {
        const intensity = Math.max(0, baseIntensity + swing * intensityAmplitude);
  ...
  filamentSensorControllers.forEach((controller) => {
    const {
      material,
      light,
      idleIntensity = 0,
      activeIntensity = idleIntensity,
      idleLight = 0,
      activeLight = idleLight,
      linkedToExtrusion = false,
      damping = 6.0,
    } = controller;
    ...
    if (material && typeof material.emissiveIntensity === 'number') {
      const target = isActive ? activeIntensity : idleIntensity;
      controller.currentIntensity = THREE.MathUtils.damp(
        controller.currentIntensity ?? idleIntensity,
        target,
        damping,
        delta,
      );
    ...
    if (light) {
      const target = isActive ? activeLight : idleLight;
      controller.currentLight = THREE.MathUtils.damp(
        controller.currentLight ?? idleLight,
        target,
        damping,
        delta,
      );
      light.intensity = controller.currentLight;
    }
  });
  cableChainPulseControllers.forEach((controller) => {
    const {
      material,
      phaseOffset = 0,
      idleIntensity = 0,
    ...
    const pulse = Math.pow(
      0.5 + 0.5 * Math.sin(elapsed * speed + phaseOffset * Math.PI * 2),
      pulseExponent,
    );
    const targetIntensity = THREE.MathUtils.lerp(idleIntensity, activeIntensity, pulse);
    controller.currentIntensity = THREE.MathUtils.damp(
      controller.currentIntensity ?? idleIntensity,
      targetIntensity,
      damping,
    ...
    material.emissiveIntensity = controller.currentIntensity;
    if (
      typeof idleOpacity === 'number' &&
      typeof activeOpacity === 'number' &&
      typeof material.opacity === 'number' &&
      material.transparent
    ) {
      const targetOpacity = THREE.MathUtils.lerp(idleOpacity, activeOpacity, pulse);
      controller.currentOpacity = THREE.MathUtils.damp(
        controller.currentOpacity ?? idleOpacity,
        targetOpacity,
        damping,
        delta,
      );
  ...
  thermistorConduitControllers.forEach((controller) => {
    ...
    const shimmer = 0.5 + 0.5 * Math.sin(elapsed * shimmerSpeed);
    const previewBoost = yarnExtrusionActive
      ? 1
      : THREE.MathUtils.clamp(upcomingFeedUrgency * countdownBias, 0, 1);
    const highlightStrength = THREE.MathUtils.clamp(shimmer * 0.6 + previewBoost, 0, 1);

    if (typeof material.emissiveIntensity === 'number') {
      const target = THREE.MathUtils.lerp(baseIntensity, activeIntensity, highlightStrength);
      controller.currentIntensity = THREE.MathUtils.damp(
    ...
    if (typeof material.opacity === 'number' && material.transparent) {
      const target = THREE.MathUtils.lerp(baseOpacity, activeOpacity, highlightStrength);
      controller.currentOpacity = THREE.MathUtils.damp(
    ...
    if (light) {
      const target = THREE.MathUtils.lerp(baseLight, activeLight, highlightStrength);
      controller.currentLight = THREE.MathUtils.damp(
        controller.currentLight ?? baseLight,
        target,
        damping,
        delta,
      );
  ...
  automationSweepControllers.forEach((controller) => {
    const {
      material,
      idleSpeed = 0.18,
      approachSpeed = idleSpeed,
    ...
    const sweepStrength = yarnExtrusionActive ? 1 : upcomingFeedUrgency;
    const speed = yarnExtrusionActive
      ? activeSpeed
      : THREE.MathUtils.lerp(idleSpeed, approachSpeed, sweepStrength);
    material.dashOffset = (material.dashOffset ?? 0) - delta * speed;

    const targetOpacity = THREE.MathUtils.lerp(baseOpacity, activeOpacity, sweepStrength);
    controller.currentOpacity = THREE.MathUtils.damp(
      controller.currentOpacity ?? baseOpacity,
      targetOpacity,
      damping,
  ...
  cableChainChaseControllers.forEach((controller) => {
    const {
      ...
      activeSpeed = approachSpeed,
      restIntensity = 0.4,
      activeIntensity = restIntensity,
      restLight = light?.intensity ?? 0,
      activeLight = restLight,
      damping = 6.0,
    } = controller;
    ...
    const targetLight = THREE.MathUtils.lerp(restLight, activeLight, chaseStrength);
    controller.currentLight = THREE.MathUtils.damp(
      controller.currentLight ?? restLight,
      targetLight,
      damping,
      delta,
    );
  ...
  anchorPulseControllers.forEach((controller) => {
    ...
    const hasSequence =
      ...
      sequenceCount > 0 &&
      Number.isFinite(sequenceDuration) &&
      sequenceDuration > 0;
    const clock = hasSequence
      ? (elapsed % sequenceDuration) / sequenceDuration
      : elapsed * speed;
    const cyclePosition = hasSequence ? clock * sequenceCount : clock + phaseOffset;
    const loopedPosition = cyclePosition % (sequenceCount || 1);
    const distance = hasSequence
      ? Math.min(
          Math.abs(loopedPosition - sequenceIndex),
          sequenceCount - Math.abs(loopedPosition - sequenceIndex),
        )
      : 0;
    const emphasis = hasSequence
      ? Math.max(0, 1 - distance)
      : Math.pow(0.5 + 0.5 * Math.sin((clock + phaseOffset) * Math.PI * 2), 2);
  ...
  const zLiftFeedHighlight = yarnExtrusionActive ? 1 : upcomingFeedUrgency * 0.7;
  zLiftGlowControllers.forEach((controller) => {
    const {
      material,
      light,
      baseIntensity = 0,
    ...
    const idlePulse = 0.5 + 0.5 * Math.sin((elapsed + phaseOffset) * speed * Math.PI * 2);
    const emphasis = THREE.MathUtils.clamp(
      idlePulse + zLiftFeedHighlight + feedBoost * zLiftFeedHighlight,
      0,
      2,
    );

    if (material && typeof material.emissiveIntensity === 'number') {
      const target = baseIntensity + amplitude * emphasis;
      controller.currentIntensity = THREE.MathUtils.damp(
        controller.currentIntensity ?? baseIntensity,
        target,
        damping,
      ...
      material.emissiveIntensity = Math.max(0, controller.currentIntensity);
    }

    if (light) {
      const targetLight = baseLightIntensity + lightAmplitude * emphasis;
      controller.currentLight = THREE.MathUtils.damp(
        controller.currentLight ?? baseLightIntensity,
        targetLight,
        damping,
        delta,
      );
      light.intensity = Math.max(0, controller.currentLight);
    }
  });
  ...
  tensionLabPathControllers.forEach((controller) => {
    const {
      tracer,
      light,
      curve,
    ...
    const previewHighlight = yarnExtrusionActive ? 1 : upcomingFeedUrgency;
    const speed = THREE.MathUtils.lerp(idleSpeed, activeSpeed, previewHighlight);
    const travel = typeof controller.travel === 'number' ? controller.travel : 0;
    const updatedTravel = (travel + delta * speed) % 1;
    controller.travel = updatedTravel;

    curve.getPointAt(updatedTravel, automationTracerPoint);
    tracer.position.copy(automationTracerPoint);
    if (light) {
      light.position.copy(automationTracerPoint).add(automationLightOffset);
    }
    ...
    tracer.material.emissiveIntensity = controller.currentIntensity;

    const targetOpacity = THREE.MathUtils.lerp(restOpacity, activeOpacity, previewHighlight);
    controller.currentOpacity = THREE.MathUtils.damp(
      controller.currentOpacity ?? restOpacity,
      targetOpacity,
      damping,
    ...
    tracer.material.opacity = controller.currentOpacity;

    if (light) {
      const targetLight = THREE.MathUtils.lerp(
        restLightIntensity,
        activeLightIntensity,
        previewHighlight,
      );
      light.intensity = THREE.MathUtils.damp(
        controller.currentLight ?? restLightIntensity,
        targetLight,
        damping,
        delta,
      );
      controller.currentLight = light.intensity;
    }
  });
  spoolCycleProgressRatio =
    patternPreviewDurationSeconds > 0
      ? THREE.MathUtils.clamp(
          (patternPreviewElapsed % patternPreviewDurationSeconds) /
            patternPreviewDurationSeconds,
          0,
          1,
        )
  ...
  if (spoolProgressSegments.length > 0) {
    ...
    spoolProgressSegments.forEach((controller, index) => {
      ...
      const prePulseStrength = (() => {
        if (!upcomingFeedUrgency) {
          return 0;
        }
        const speed = spoolPrePulseSettings.speed ?? 0;
        const phaseOffset = spoolPrePulseSettings.phaseOffset ?? 0;
        const phase = elapsed * speed + phaseOffset * index;
        return upcomingFeedUrgency * (0.5 + 0.5 * Math.sin(phase));
      })();
      const highlightedStrength = THREE.MathUtils.clamp(
        targetStrength +
          feedPulseHighlight * feedPulseWave * 0.6 +
          prePulseStrength * 0.6,
        0,
        1,
      );
  ...
  if (spoolCycleSegments.length > 0) {
    const cycleProgress = THREE.MathUtils.clamp(spoolCycleProgressRatio, 0, 1);
    const scaledCycle = cycleProgress * spoolCycleSegments.length;
    const fullCycleSegments = Math.floor(scaledCycle);
    const partialCycleStrength = scaledCycle - fullCycleSegments;
    const cyclePulse = 0.4 + 0.6 * Math.sin(elapsed * 2.6);
    spoolCycleSegments.forEach((controller, index) => {
      const {
        mesh,
        material,
        baseIntensity = 0,
      ...
      let targetStrength = 0;
      if (index < fullCycleSegments) {
        targetStrength = 1;
      } else if (index === fullCycleSegments) {
        targetStrength = partialCycleStrength;
      }
      const highlighted = THREE.MathUtils.clamp(targetStrength + cyclePulse * 0.35, 0, 1);
      const intensityTarget =
        baseIntensity + (activeIntensity - baseIntensity) * highlighted;
      controller.currentIntensity = THREE.MathUtils.damp(
        controller.currentIntensity ?? baseIntensity,
        intensityTarget,
        damping,
      ...
      material.emissiveIntensity = controller.currentIntensity;
      const opacityTarget = baseOpacity + (activeOpacity - baseOpacity) * highlighted;
      controller.currentOpacity = THREE.MathUtils.damp(
        controller.currentOpacity ?? baseOpacity,
        opacityTarget,
        damping,
  ...
  servoSignalControllers.forEach((controller) => {
    const {
      ...
      restIntensity = 0.3,
      activeIntensity = 1.0,
      restLightIntensity = controller.restLightIntensity ?? light?.intensity ?? 0,
      activeLightIntensity =
        controller.activeLightIntensity ?? light?.intensity ?? 0,
    ...
    if (light) {
      const targetLight = yarnExtrusionActive ? activeLightIntensity : restLightIntensity;
      controller.currentLightIntensity = THREE.MathUtils.damp(
        controller.currentLightIntensity ?? targetLight,
        targetLight,
        damping,
        delta,
      );
      light.intensity = controller.currentLightIntensity;
    }
  ...
  controllerStackGlowControllers.forEach((controller) => {
    const {
      material,
      restIntensity = 0.5,
      activeIntensity = 1.1,
      idlePulseStrength = 0,
      restOpacity = controller.restOpacity ?? material?.opacity ?? 1,
      activeOpacity = controller.activeOpacity ?? restOpacity,
      pulseSpeed = 2.2,
      phaseOffset = 0,
      damping = 5.6,
    } = controller;
    ...
    const pulse = 0.5 + 0.5 * Math.sin(elapsed * pulseSpeed + phaseOffset);
    const pulseStrengthRaw = yarnExtrusionActive ? 1 : idlePulseStrength;
    const pulseStrength = THREE.MathUtils.clamp(pulseStrengthRaw, 0, 1);
    const targetIntensity = THREE.MathUtils.lerp(
      restIntensity,
      activeIntensity,
      pulse * pulseStrength,
  ...
  statusIndicatorControllers.forEach((controller) => {
    const {
      material,
      light,
      idleIntensity = 0,
    ...
    const targetIntensity =
      (yarnExtrusionActive ? activeIntensity : idleIntensity) + pulse;
    const targetLight = (yarnExtrusionActive ? activeLight : idleLight) + pulse;
    ...
    if (light) {
      controller.currentLight = THREE.MathUtils.damp(
        controller.currentLight ?? targetLight,
        targetLight,
        damping,
        delta,
      );
  ...
  updatePatternPreview(patternPreviewElapsed);
...
function loadPlannerFile(file) {
  if (!file) {
    return;
  }

  if (plannerUploadResetTimer) {
    clearTimeout(plannerUploadResetTimer);
  }

  if (file.size > plannerUploadLimitBytes) {
    updateStatus('Planner upload too large — please use files under 10MB.');
    plannerUploadResetTimer = window.setTimeout(resetPlannerUploadField, 750);
    return;
  }
  ...
  reader.onload = () => {
    try {
      ...
      const events = eventsFromPlannerPayload(payload);
      if (!events.length) {
        throw new Error('No planner commands found');
      }
      ...
      if (!metadata.source) {
        metadata.source = `Upload: ${file.name}`;
      }
      ...
      updateMachineProfilePanel(machineProfile);
      updateStatus(`Planner upload loaded: ${file.name}`);
    } catch (error) {
      console.warn('Unable to load uploaded planner', error);
      updateStatus('Planner upload failed — see console for details.');
    } finally {
      plannerUploadResetTimer = window.setTimeout(resetPlannerUploadField, 750);
    }
  };
  ...
  reader.onerror = () => {
    ...
    console.warn('Unable to read uploaded planner');
    updateStatus('Planner upload failed — see console for details.');
    plannerUploadResetTimer = window.setTimeout(resetPlannerUploadField, 750);
  };
...
function handlePlannerUpload(event) {
  const file = event?.target?.files?.[0];
  if (!file) {
    return;
  }

  loadPlannerFile(file);
}
...
function handlePlannerDragOver(event) {
  ...
  // Only show hover state when a JSON file is being dragged
  const items = event.dataTransfer.items;
  if (items && items.length > 0 && items[0].type && items[0].type !== 'application/json') {
    return;
  }
  event.preventDefault();
  event.dataTransfer.dropEffect = 'copy';
  setPlannerUploadHover(true);
}
...
function handlePlannerDrop(event) {
  ...
  if (file) {
    ...
    if (isJsonType || isJsonExt) {
      ...
      updateStatus('Error: Only JSON files are supported for planner upload.');
    }
  }
}
...
window.addEventListener('drop', handlePlannerDrop);
window.addEventListener('dragend', handlePlannerDragLeave);
